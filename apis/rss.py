...
async def get_mp_articles_source(
    ...
    try:
        ...
        rss_list = [{
            ...
            # link 指向空锚点，阻止 RSS 阅读器二次抓取导致内容重复显示
            "link": "#",
            # 只放全文到 description，不生成 content:encoded，避免阅读器重复显示
            "description": (article.content or "")[:50000] if (str(article.has_content) == "1" and article.content) else (article.description if article.description != "" else article.title or ""),
            # content 字段留空，core/rss.py 不再生成 content:encoded
            "content": "",
