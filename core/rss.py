...
class RSS:
    ...
    def generate_rss(self,rss_list: dict, title: str = "Mp-We-Rss", 
        ...
        # 不设置 xmlns:content，避免 ET 自动序列化为 content:encoded
        ...
        for rss_item in rss_list:
            ...
            # description 放全文 HTML，用 CDATA 防止转义
            _desc = str(rss_item["description"] or "")
            desc_elem = ET.SubElement(item, "description")
            desc_elem.text = ET.CDATA(_desc)
            ...
            # 不生成 content:encoded，只使用 description 字段，避免阅读器重复显示
            # _content = rss_item.get("content")
            # if _content:
            #     ce = ET.SubElement(item, "{http://purl.org/rss/1.0/modules/content/}encoded")
            #     ce.text = ET.CDATA(_content)
        ...
        tree_str = '<?xml version="1.0" encoding="utf-8"?>\r\n' + \
                ET.tostring(rss, encoding="utf-8").decode("utf-8")
    ...
    def generate_atom(self,rss_list: dict, title: str = "Mp-We-Rss", 
        ...
        # 不设置 xmlns:content，避免 lxml 自动序列化为 content:encoded
        # if full_context==True:
        ...
        for rss_item in rss_list:
            ...
            desc_text = str(rss_item["description"] or "")
            ET.SubElement(entry, "summary").text = ET.CDATA(desc_text)
            ...
            # Atom 标准：summary 放摘要，content 放全文
            if full_context == True and rss_item.get("content"):
                content_elem = ET.SubElement(entry, "content")
                content_elem.set("type", "html")
                content_elem.text = rss_item["content"]
        # 生成XML字符串
        tree_str = '<?xml version="1.0" encoding="utf-8"?>\r\n' + \
