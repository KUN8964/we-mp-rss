    ...
    try:
        ...
        for tag in tags:
            ...
            tag_data = {
                "id": tag.id,
                "name": tag.name,
                "cover": _get_web().get_image_url(tag.cover) if tag.cover else "",
                "intro": tag.intro,
                "mp_count": mp_count,
...
async def tag_detail_view(
    ...
    try:
        ...
        if mps_ids:
            ...
            for article, feed in articles_query:
                article_data = {
                    ...
                    "pic_url": _get_web().get_image_url(article.pic_url),
                    "mp_cover": _get_web().get_image_url(feed.mp_cover) if feed else "",
                    "url": article.url,
        ...
        tag_data = {
            ...
            "article_count": total,
            "sync_time": datetime.fromtimestamp(tag.sync_time).strftime('%Y-%m-%d %H:%M') if tag.sync_time else "未同步",
            "mps": [{"id": mp.id, "name": mp.mp_name, "cover": _get_web().get_image_url(mp.mp_cover)} for mp in mps_info]
        }
